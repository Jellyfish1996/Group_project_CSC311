...
def build_bagged_ensemble(
    ...
    ext_k: int = 602, ext_lr: float = 0.0846, ext_iters: int = 109,
    # IRT hyperparams
    ...
    # (load_train_sparse used only if you want dims quickly, but keep for symmetry)
    _ = load_train_sparse("./data").toarray()
    ...
    # track per-bag accuracies for each base model
    ...
    for b in range(1, B + 1):
        ...
        # Extended ALS with (theta, beta, bias)
        ...
        # Collect predictions for this bag
        preds = {"ALS": M_als, "ExtALS": M_ext, "IRT": M_irt}

        # Per-model accuracies on val and test
        for nm, mat in preds.items():
            model_val_accs[nm].append(sparse_matrix_evaluate(val, mat))
            model_test_accs[nm].append(sparse_matrix_evaluate(test, mat))

        # Accuracy-weighted blend (on validation)
        blend_val, accs_val, weights = blend_by_val_accuracy(preds, val)
        blend_test = sum(weights[name] * preds[name] for name in preds)

        v = sparse_matrix_evaluate(val, blend_val)
        t = sparse_matrix_evaluate(test, blend_test)
        ...
        # running mean of blended predictions (for a final stable matrix)
    ...
    final_val  = sparse_matrix_evaluate(val,  blended_val_running)
    final_test = sparse_matrix_evaluate(test, blended_test_running)
    ...
    plt.boxplot([bag_val, bag_test], labels=["Val", "Test"])
    plt.ylabel("Accuracy")
    plt.title("Per-bag accuracy distribution")
    plt.tight_layout()
    plt.savefig("figs/bag_accuracy_boxplot.png", dpi=150)
    plt.close()

    # 1b) Generalization gap: (Validation - Test) per bag for each base model and the ensemble
    plt.figure(figsize=(7.6, 4.6))
    labels = ["ALS", "ExtALS", "IRT", "Ensemble"]
    gap_series = [
        np.array(model_val_accs["ALS"])    - np.array(model_test_accs["ALS"]),
        np.array(model_val_accs["ExtALS"]) - np.array(model_test_accs["ExtALS"]),
        np.array(model_val_accs["IRT"])    - np.array(model_test_accs["IRT"]),
        np.array(bag_val) - np.array(bag_test),  # ensemble
    ]
    ...
    plt.tight_layout()
    plt.savefig("figs/val_minus_test_gap_per_model.png", dpi=150)
    plt.close()

    # 2) Scatter: Val vs Test per bag (generalization)
    plt.figure(figsize=(5.6, 5.2))
    plt.scatter(bag_val, bag_test, s=40)
    lims = [min(bag_val + bag_test) - 0.01, max(bag_val + bag_test) + 0.01]
    plt.plot(lims, lims, "--")
    plt.xlabel("Validation accuracy")
    plt.ylabel("Test accuracy")
    plt.title("Generalization: per-bag Val vs Test")
    plt.tight_layout()
    plt.savefig("figs/bag_val_vs_test_scatter.png", dpi=150)
    plt.close()

    # 3) Cumulative mean test accuracy vs number of bags (stability)
    cum_mean = np.cumsum(bag_test) / (np.arange(len(bag_test)) + 1)
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig("figs/cumulative_mean_test_accuracy.png", dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("figs/avg_model_weights.png", dpi=150)
    plt.close()

    # 5) Calibration on validation (reliability curve) for final blended matrix
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("figs/calibration_validation.png", dpi=150)
    plt.close()

    print("\n=== Bagged Ensemble Summary ===")
    print(f"Val mean±std:  {np.mean(bag_val):.4f} ± {np.std(bag_val):.4f}")
    print(f"Test mean±std: {np.mean(bag_test):.4f} ± {np.std(bag_test):.4f}")
    print(f"Final blended  Val={final_val:.4f}  Test={final_test:.4f}")
    print("Average model weights:", avg_weights)
    print("Saved: figs/* and JSON summary ->", f"{save_prefix}_summary.json")
...
if __name__ == "__main__":
    build_bagged_ensemble(
        B=20,
        als_k=50, als_lr=0.3, als_iters=8000,
        ext_k=602, ext_lr=0.0846, ext_iters=109,
